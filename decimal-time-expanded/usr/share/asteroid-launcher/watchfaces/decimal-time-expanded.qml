...
Item {
    ...
    readonly property string timerRunningConfigKey: "/org/asteroidos/timer/running"
    readonly property string timerRemainingSecondsConfigKey: "/org/asteroidos/timer/remaining-seconds"
    ...
    ConfigurationValue {
        id: timerRemainingSeconds
        key: timerRemainingSecondsConfigKey
        defaultValue: -1
    }
    ...
    PlainText {
        id: runningTimer
        text: timerRemainingSeconds.value ?? ""
        // explicitly comparing to true, because it might be undefined
        // at first, since ConfigurationValue loads its value asynchronously
        visible: timerRunning.value === true
        anchors.verticalCenterOffset: +parent.width*0.26

        Connections {
            target: timerRemainingSeconds
            function onValueChanged() {
                runningTimer.text = timerRemainingSeconds.value ?? ""
            }
        }
    }
